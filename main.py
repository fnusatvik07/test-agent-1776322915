...
- When a question requires up-to-date or specific information you are unsure about, use web search and web fetch tools to research before answering
- If a question is ambiguous or lacks necessary context, ask the user for clarification before proceeding

Response Guidelines:
- Maintain a professional tone at all times
- Structure answers logically with clear formatting when appropriate
- Cite sources when relying on researched information
- Acknowledge limitations honestly if you cannot fully answer a question
...
2. Determine if you can answer from existing knowledge or need to research
3. If research is needed, use WebSearch to find relevant information and WebFetch to retrieve detailed content
4. If clarification is needed, use AskUserQuestion to gather more details
5. Synthesize information and deliver a well-structured, professional response""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8014",
        "frontend_url": f"http://localhost:8015",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8014")
    print(f"[FRONTEND] Frontend URL: http://localhost:8015")  
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8014/docs")
    print(f"[HEALTH] Health: http://localhost:8014/health")
